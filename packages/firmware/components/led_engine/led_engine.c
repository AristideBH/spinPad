...
static uint8_t             g_brightness    = 255;         // Luminosité globale
static uint8_t             g_ext_rgb[LED_EXT_MAX * 3];    // Extension frame (R,G,B,R,G,B,...)
static uint8_t             g_ext_count     = 0;
...
static void render_frame(void)
{
    ...
    // Extension de chaîne
    for (uint8_t i = 0; i < g_ext_count && i < LED_EXT_MAX; i++) {
        uint8_t r = scale8(g_ext_rgb[i * 3 + 0], g_brightness);
        uint8_t g = scale8(g_ext_rgb[i * 3 + 1], g_brightness);
        uint8_t b = scale8(g_ext_rgb[i * 3 + 2], g_brightness);
...
void led_engine_tick(void)
{
    ...
    bool should_render = g_dirty || (need_breathe && (g_tick_count % REFRESH_EVERY_TICKS == 0));
