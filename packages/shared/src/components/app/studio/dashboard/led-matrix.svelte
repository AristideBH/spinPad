<script lang="ts">
  ...
  function paletteAt(pal: RGB[], f: number): RGB {
    if (pal.length === 1) return pal[0];
    const x = clamp01(f) * (pal.length - 1);
    const i = Math.floor(x);
    return lerp(pal[i], pal[Math.min(i + 1, pal.length - 1)], x - i);
  }
  ...
  function setup(canvas: HTMLCanvasElement) {
    ...
    function draw(now: number): boolean {
      ...
      const kA = snap ? 1 : 1 - Math.exp(-dt / TAU_A);
      const kC = snap ? 1 : 1 - Math.exp(-dt / TAU_C);
      const cap = brightness * (muted ? 0.28 : 1);
      const r0 = (dotRatio * cell) / 2;
      const pal = colors.length ? colors.map(rgb) : ([[255, 255, 255]] as RGB[]);

      // ── EQ : ingestion + gravité ────────────────────────────────────
      const eqOn = !!pulses;
      let eMax = 0;
      if (eqOn) {
        ingest();
      ...
      const inv2s2 = 1 / (2 * EQ_SPREAD * EQ_SPREAD);

      // ── Vagues encodeur : ingestion + vieillissement ──
      if (rotation) {
        ingestRot(now);
      ...
      const invW2 = 1 / (2 * WAVE_SIGMA * WAVE_SIGMA);
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        let col: RGB =
          mode === 'rainbow'
            ? hslToRgb((d.nx * 300 + t * 60) % 360, 0.85, 0.6)
            : color === 'solid'
              ? pal[0]
              : paletteAt(pal, d.nx);
        ...
        // ── Vague horizontale directionnelle (encodeur) ──
        for (let w = 0; w < waves.length; w++) {
          const prog = (now - waves[w].born) / 1000 / WAVE_DUR; // 0..1
          const front = waves[w].dir > 0 ? prog : 1 - prog;
          const wx = d.nx - front;
          tv += (1 - prog) * WAVE_AMP * eqGain * Math.exp(-(wx * wx) * invW2);
        }

        const ta = clamp01(cap * tv);

        // easing vers la cible : alpha rapide (kA), couleur lente (kC)
        const da = ta - dA[i];
        ...
        dG[i] += (col[1] - dG[i]) * kC;
        dB[i] += (col[2] - dB[i]) * kC;
        const resid = Math.abs(da) > dc ? Math.abs(da) : dc;
        ...
        ctx!.arc(d.x, d.y, glow ? r0 * (0.8 + 0.5 * a) : r0, 0, Math.PI * 2);
        ctx!.fill();
      }
