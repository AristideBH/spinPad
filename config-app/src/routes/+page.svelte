<script>
    import { connect, disconnect, factoryReset, serial } from '$lib/serial/index.svelte.js';
    import { loadConfig, saveConfig, configState }       from '$lib/store/config.svelte.js';
...
{#if !isOnline}
    ...
    <div class="flex flex-col items-center gap-4 mt-4">
        ...
        <div class="mt-2 text-center">
            ...
            <p class="text-xs text-muted-foreground mt-1">Prérequis : Chrome ou Edge, clavier USB. WebSerial non supporté sur Firefox.</p>
        </div>
    </div>

{:else if configState.isLoading}
    <p class="text-center text-muted-foreground mt-10 text-sm">Chargement de la config...</p>

{:else if configState.data}
    <!-- Info grid -->
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        <InfoCard label="Profils"       value={configState.data.profile_count ?? configState.data.profiles?.length} />
        <InfoCard label="Profil actif"  value={configState.data.profiles?.[configState.data.active_profile]?.name} />
        <InfoCard label="BLE Device"    value={configState.data.ble?.slot_names?.[configState.data.ble?.active_slot]} />
        <InfoCard label="Sleep timeout" value="{configState.data.power?.sleep_timeout_s}s" />
